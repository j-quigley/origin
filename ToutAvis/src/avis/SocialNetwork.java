...
public class SocialNetwork {
	...
	public void addItemFilm(String pseudo, String password, String titre, String genre, String realisateur, String scenariste, int duree) throws BadEntry, NotMember, ItemFilmAlreadyExists {
		...
		//test si le membre est pr�sent dans la liste
		if (m == null) throw new NotMember("Not member : Membre non pr�sent");
		//Test du password si le membre existe 
		if(!m.isPassword(password)) throw new NotMember("Password erron�");
		// si le membre est pr�sent, on continue
		else {
		...
		for(Item i : items){
			if(i instanceof ItemFilm){
				if(((ItemFilm) i).getTitre().trim().toLowerCase().equals(nom.trim().toLowerCase())){
					note = i.getMoyenne();
					if(note>=0)
						result.add("Film trouv� : Titre : "+((ItemFilm) i).getTitre()+" genre : "+((ItemFilm) i).getGenre()+" note : "+note);
					else result.add("Film trouv� : Titre : "+((ItemFilm) i).getTitre()+" genre : "+((ItemFilm) i).getGenre());
				}
			}
			if(i instanceof ItemBook){
				if(((ItemBook) i).getTitre().trim().toLowerCase().equals(nom.trim().toLowerCase())){
					note = i.getMoyenne();
					if(note>=0)
						result.add("Livre trouv� : Titre : "+((ItemBook) i).getTitre()+" genre : "+((ItemFilm) i).getGenre()+"note : "+note);
					else result.add("Livre trouv� : Titre : "+((ItemBook) i).getTitre()+" genre : "+((ItemFilm) i).getGenre());
				}
			}
		}
		return result;
	...
	public float reviewItemFilm(String pseudo, String password, String titre, float note, String commentaire) throws BadEntry, NotMember, NotItem {
		...
		//Ajout du review
		Review r = new Review(m, note, commentaire);
		j.addReview(r);
		return 0.0f;
	}
